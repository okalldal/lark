...
fn test_scanner_backends_lex_identically() {
    ...
    // Likewise the python.lark differential runs *with LONG_STRING lowered* (the multi-char
    // `"""` close, DOTALL) — so the Python-files corpus above (docstrings + triple-quoted
    // literals) is the lowered DFA agreeing with fancy, not two fancy side-probes agreeing.
