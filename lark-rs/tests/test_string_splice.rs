...
/// executable). The honest scope on master is: `python.STRING` lowers into the DFA (the
/// M4 splice) and `lark.REGEXP` lowers via the regex-literal delimited-token idiom
/// (Stage B, `recognize_regexp_idiom` — gated by `tests/test_regexp_splice.rs`, the
/// state-pruned Route-1 proof, and the generative-equivalence layer), while
/// `python.LONG_STRING` (a lazy `.*?` body with a multi-character `"""` close and no
/// opening guard) still **declines** — it routes to `fancy-regex`, so `fancy-regex`
/// stays in the runtime and L4/L5 remain blocked.
///
...
/// correct and (b) re-run the payoff-check: with *all* bundled lookaround terminals
/// lowered, L4 (drop `AnyRegex::Fancy` from the runtime — which *also* needs the
/// `Unsupported` compatibility-fallback policy flip) and L5 (bake) become unblocked, and
...
/// assert a permanent rejection, because lowering it is a planned Stage-B follow-up
...
fn bundled_lookaround_terminal_lowering_status() {
    ...
    // STRING and REGEXP lower (the M4 splice and the Stage-B regex-literal idiom).
    assert!(
        matches!(
    ...
    // LONG_STRING is declined (routes to fancy). A returned `Branches` here means the
    // scope changed — see the doc above: prove it and re-run the L4/L5 payoff. If this
    // starts lowering, that is good news ONLY if the PR also adds equivalence/proof/
    // canary coverage and updates the L4/L5 status (plan + STATUS.md).
    assert!(
        !matches!(
    ...
    assert!(
        ...
        "python.LONG_STRING must still compile on fancy-regex (declined-to-fancy, not rejected)"
    );
}
