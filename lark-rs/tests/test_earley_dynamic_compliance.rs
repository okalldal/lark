//! Earley **dynamic-lexer** compliance bank: replays grammars/inputs strip-mined
//! from Python Lark's dynamic-lexer Earley test classes
//! (tools/extract_lark_compliance.py::run_earley_dynamic_suite) and checks lark-rs
//! against the captured oracle trees and construct-error outcomes.
//!
//! This is the Sprint-5 regression net, burned down exactly as the basic-lexer
//! Earley bank (test_earley_compliance.rs) was. Gated by an XFAIL allow-list
//! (compliance/earley_dynamic_xfail.json): the build fails only on a *regression*
//! — a case that newly fails and is not allow-listed. Regenerate the allow-list
...
fn record_options(rec: &Value) -> LarkOptions {
    ...
    // The dynamic bank's distinguishing dimension: dynamic vs dynamic_complete.
...
#[test]
fn test_earley_dynamic_compliance_bank() {
    let Some(records) = load_json("earley_dynamic_bank.json") else {
        eprintln!("earley_dynamic_bank.json not found — run tools/extract_lark_compliance.py");
        return;
    };
    ...
    for (ri, rec) in records.iter().enumerate() {
        ...
        if trace {
            eprintln!(
                "[{ri}] lexer={:?} ambiguity={:?} grammar={:?}",
                rec["lexer"], rec["ambiguity"], grammar
            );
        }
    ...
    let xfail: BTreeSet<String> = load_json("earley_dynamic_xfail.json")
    ...
    eprintln!(
        "earley dynamic compliance bank: {passing}/{total} agree with oracle ({pct:.1}%); \
         {} known-XFAIL{}",
        xfail.len(),
        if unimplemented {
    ...
    if std::env::var("LARK_COMPLIANCE_WRITE_XFAIL").is_ok() {
        ...
        let path = fixtures_dir().join("earley_dynamic_xfail.json");
        std::fs::write(&path, serde_json::to_string_pretty(&list).unwrap() + "\n")
            .expect("write earley_dynamic_xfail.json");
        eprintln!(
    ...
    if !fixed.is_empty() {
        eprintln!(
            "note: {} dynamic XFAIL entries now pass — consider regenerating earley_dynamic_xfail.json",
            fixed.len()
        );
    }
    assert!(
        regressions.is_empty(),
        "earley dynamic compliance regressions ({} cases newly failing and not in earley_dynamic_xfail.json):\n{}",
        regressions.len(),
        regressions
            .iter()
            .take(40)
