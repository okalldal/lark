...
/// The **long-string idiom** (`python.LONG_STRING`'s shape — a multi-character `"""`/`'''`
/// close with no opening guard): every generated idiom terminal's lowered match-length must
/// equal the `fancy-regex` oracle over its exhaustive corpus — which reaches complete long
/// strings and exercises the `""" """` overshoot boundary, interior quote runs, escaped
/// quotes, and escaped backslashes. A maximal-munch overshoot (the body greedily swallowing
/// an interior close run) would diverge here. `lowered_prefix` returning `Err` (a declined
