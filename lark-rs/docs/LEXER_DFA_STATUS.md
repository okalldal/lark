...
| `python.LONG_STRING` | `…(""".*?(?<!\\)(\\\\)*?"""\|…)` | **Declined-to-fancy** | runs on `fancy-regex`; equivalence pinned by `test_lookaround.rs` | audited **delimited-token** long-string idiom (Stage B) with a multi-char `"""` delimiter automaton — the last bundled decliner | **yes** |
...
fallback. With `lark.REGEXP` now lowered, the two remaining gates are: lower
`python.LONG_STRING` (the last bundled decliner), *and* flip the `Unsupported` route to a
build error. See
