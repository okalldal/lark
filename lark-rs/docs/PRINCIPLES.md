...
2. **Python Lark is the oracle.** Expected trees/errors come from Python Lark via
   ...
   Fidelity targets Python's *intended behavior*, not its implementation artifacts:
   a divergence that is mere leakage of how CPython-Lark is built may be declined
   when matching is costly — but only deliberately, with an ADR and a pinning test
   (ADR-0017, §3).
...
- **Match the oracle's *intended* behavior, not its implementation artifacts**
  (ADR-0017). Route any observed divergence on two axes — *intentional contract vs.
  circumstantial leakage* × *cheap vs. expensive to match*. Match in every cell
  except **circumstantial + expensive**, which is "diverge & document" (an ADR + a
  pinning test). Corollary: being *more permissive* than the oracle (accepting input
  Python rejects) is unfalsifiable — match the rejection unless a documented reason
  says otherwise. Deciding which axis a behavior sits on is itself a "decide + record"
  call (§4), not an escalate-by-default.
...
- **Behaviour is scoped to Python Lark's** (ADR-0026). Ship behaviour you can
  ...
  that order; never "curated for everything"). Being behaviourally *richer* than the
...
|---|---|---|
| **Correctness & Python parity** | Oracle + compliance/wild banks (§2.1–2.2) — *the dominant axis* | A divergence from the oracle → route by ADR-0017 (intended contract vs. impl artifact); escalate only the genuinely undecidable. (#101/#159 resolved this way) |
