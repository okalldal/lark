...
src/
  ...
  standalone.rs       Standalone parser generation (#42): bake ParseTable + lexer
                      → self-contained Rust source (basic lexer + LALR driver)
...
| Standalone parser gen | ✅ (Rust) | `lark-rs generate-parser --grammar foo.lark --output parser.rs` (`src/bin/generate_parser.rs`) emits a self-contained Rust LALR parser depending only on `regex` + std, not on lark-rs (#42). `src/standalone.rs` runs the normal pipeline once and bakes the `ParseTable` (sparse ACTION/GOTO), per-rule tree-shaping flags, the symbol-name table, and the `ScannerPlan` (alternation order + `unless` retype) into `const` data, then appends a fixed runtime shim (basic lexer + LALR driver + tree-builder, trimmed copies of the in-process modules). Because the lexer data comes from the **shared** `lexer::scanner_plan` and the driver applies the same reduce/shape rules, a generated parser is byte-identical to lark-rs — pinned by `test_standalone.rs` (committed `tests/standalone/*.rs` fixtures `include!`d + run vs the live oracle, plus a determinism/freshness gate). Limitations: LALR + basic lexer only, no postlex (rejected with a clear error). Python standalone is the remaining follow-up |
