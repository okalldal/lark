...
**lowered** via the M4 opening-guard splice and `lark.REGEXP` via the Stage-B regex-literal
idiom (`recognize_regexp_idiom`); grammars stay verbatim, not rewritten — while
`python.LONG_STRING` still **declines to `fancy-regex`** (see the routing note above and
`docs/LEXER_DFA_STATUS.md`). *Historical:* the earlier
...
`""""` is a lex error, `"" ""` is two empty STRINGs). `lark.REGEXP` is **lowered** too via
the Stage-B regex-literal idiom (`recognize_regexp_idiom`: `/ body / flags` with the
internal `(?!\/)` reduced to a non-empty `+` and the lazy close proven equal to a greedy
`+`), gated by `tests/test_regexp_splice.rs` + a state-pruned Route-1 proof + generative
equivalence + the differential. **Still on the `fancy-regex` side-probe (a *decline*, never
mis-lowered):** `python.LONG_STRING` (a lazy `.*?` body with a multi-character `"""` close
and no opening guard) is attempted and declined cleanly, routed to `fancy-regex` under
**both** backends; lowering it is a follow-up the STRING/REGEXP milestones do not require (so
...
assertion → DFA branches + guard tables, M1/M2/M3/M4 + Stage-B idioms —
`STRING`/`REGEXP`/`OP`/`DEC_NUMBER` and fixed-offset lookbehind are here), *DeclinedToFancy*
(`python.LONG_STRING`, or a per-instance decline → still on `fancy-regex` at runtime), or
*Unsupported* (out-of-shape — user internal/unbounded/backref/etc. lookaround). The
decline-vs-reject split PR #131 flagged is now in the type; what is **transitional** is the
runtime policy: the *Unsupported* arm still routes to `fancy-regex` via a single auditable
`push_fancy_fallback` compatibility seam, so an out-of-shape user assertion lexes today rather
than erroring — L4 must flip only that arm to a build error. Because one bundled terminal
(`python.LONG_STRING`) still declines, **`fancy-regex` remains a runtime dependency and L4
...
> opening-guard splice and `REGEXP` via the Stage-B regex-literal idiom (not rewritten, not
> routed to fancy), while `LONG_STRING` is **declined-to-fancy** pending its own audited
> delimited-token idiom (Stage B), not a grammar edit. The behavioral findings are still
> pinned by `tests/test_lookaround.rs`.
...
`python.lark`'s `STRING`/`REGEXP`, today `LONG_STRING`) is O(n²) over the input (a 124 KB
...
Measured cost (`cargo bench --bench redos`): the shipped declined terminals stay **linear**
