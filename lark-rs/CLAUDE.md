...
`STRING`/`LONG_STRING` (the `("(?!"").*?(?<!\\)(\\\\)*?"…)` escaped-quote+triple-quote
forms) to lookaround-free escaped-string regex that run on the linear `regex` engine —
the triple-quote disambiguation moved to `LONG_STRING.2` priority, with one residual
one-directional divergence on never-valid-Python quote-runs (proven & pinned in
`tests/test_lookaround.rs`). The *still*-lookaround terminals (`DEC_NUMBER`'s
`(?![1-9])`, `lark.OP`/`REGEXP`) are the remaining E2 work; until they land (and
`fancy-regex` is dropped in E4) lark-rs handles them by **routing only the
lookaround-bearing terminals to `fancy-regex`** (a
...
`STRING`/`LONG_STRING`; these are the standing exceptions to "verbatim upstream."
