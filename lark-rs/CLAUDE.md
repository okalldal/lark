...
| Grammar standard library | ✅ | Beyond `common.lark`, lark-rs bundles every grammar Python Lark ships under `lark/grammars/` — `python.lark`, `unicode.lark`, and `lark.lark` — under `src/grammars/`, resolvable via the same `%import <lib>.<X>` directive. The files are **near-verbatim** copies: the loader's bundled-library path parses each through lark-rs's own loader and copies the requested terminal/rule closure, mangled under the module prefix (`python__HEX_NUMBER`). A handful of their terminals still use lookaround, which the lexer transparently routes to `fancy-regex` (see the parity-gap note below). **Lookaround elimination (docs/LOOKAROUND_ELIMINATION_PLAN.md):** milestone E2a rewrote `python.lark`'s `LONG_STRING` to a lookaround-free escaped-string form so it rejoins the fast `regex` scanner (the lazy `"""(?:[^\\]|\\.)*?"""` body is byte-for-byte equivalent to upstream's lookbehind even-backslash guard — proven by `tests/test_lookaround.rs::long_string_match_length_equivalence`). `STRING` is **left verbatim** on its lookaround form: its `(?!"")` opening guard is an irreducible *trailing-context* boundary (it rejects `""""` while accepting `"" ""`, a distinction lost once `%ignore` drops whitespace), so no lookaround-free rewrite reproduces the oracle — the negative result is pinned by `string_lookaround_free_rewrite_is_not_equivalent`. Pinned by `tests/test_stdlib.rs` (oracles in `fixtures/oracles/stdlib/`). SQL/C/Lua are *not* bundled — upstream distributes them as separate packages, not under `lark/grammars/` |
...
on lookaround. lark-rs is **eliminating** these at grammar-load time
(docs/LOOKAROUND_ELIMINATION_PLAN.md): milestone **E2a** rewrote `python.lark`'s
`LONG_STRING` to a lookaround-free escaped-string regex (proven byte-for-byte
equivalent, `tests/test_lookaround.rs`). `STRING` **could not** be eliminated — its
`(?!"")` opening guard is an irreducible trailing-context boundary (any rewrite accepts
`""""`, which diverges from the oracle), so it stays on lookaround; this is the
"boundary-as-failure" case the plan flags. `DEC_NUMBER`'s `(?![1-9])` and
`lark.OP`/`REGEXP` are the remaining E2 candidates. Until the irreducible set is
resolved (and `fancy-regex` is dropped in E4) lark-rs handles every still-lookaround
terminal by **routing only the lookaround-bearing terminals to `fancy-regex`** (a
...
*sparse* lookaround terminal (historically `python.lark`'s `STRING`, before E2a moved
it off `fancy-regex`; now e.g. `lark.lark`'s `REGEXP`) is O(n²) over the input (a
124 KB Python file took ~177 s before this was fixed; the pure-`regex` JSON/SQL
...
redos`): the remaining lookaround terminals stay **linear** — `fancy-regex` runs their
ambiguous bodies on the linear engine and only backtracks around the fixed leading
assertion, so there is no ReDoS. Two terminals keep a hand-written lookaround-free
form: `common.lark`'s `ESCAPED_STRING` (the hottest terminal in the library, already
linear on the pure `regex` engine) and — as of milestone E2a — `python.lark`'s
`LONG_STRING`; these are the standing exceptions to "verbatim upstream." (`python.lark`'s
`STRING` stays verbatim on lookaround — see E2a above: its `(?!"")` guard is
irreducible.)
