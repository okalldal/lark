...
//! `python.STRING`'s `(?!"")` sits *after a variable-width prefix + the opening quote*, and
//! `lark.REGEXP`'s `(?!\/)` sits *after the opening slash* — positions the top-level walk
//! sees as `Internal`. They lower only because the [`super::lower`] idiom recognizers
//! ([`super::lower::recognize_string_idiom`] / [`super::lower::recognize_regexp_idiom`])
//! match those *exact* terminal shapes and re-tag their interior lookaheads as `Leading`;
//! outside a recognized idiom a deeper lookahead stays `Internal` (reject). The recognizer —
//! never a position heuristic — is the single gate, so the dangerous direction
//! (false-accept) stays closed. One bundled lookaround terminal is *not* lowered yet and
//! **declines to `fancy-regex`**: `python.LONG_STRING` (multi-char `"""` close); see
...
pub fn classify(pattern: &str) -> Result<Classification, GrammarError> {
    ...
    // Delimited-token idiom refinement (python.STRING, lark.REGEXP): an interior opening
    // guard is seen by the top-level walk as `Internal` — STRING's `(?!"")` sits after a
    // variable-width prefix + the opening quote, and REGEXP's `(?!\/)` sits after the
    // opening slash (`docs/LEXER_DFA_PLAN.md`, Stage B). Both are, however, *supported*
    // leading boundaries the [`super::lower`] idiom recognizers splice/strip away. So
    // **only when the whole terminal is a recognized, lowerable idiom**, re-tag its
    // interior `(?=)/(?!)` lookaheads as `Leading`. Outside a recognized idiom the verdict
    // is unchanged, so a genuinely-internal lookahead (`a(?=b)c`, the verilog `(?!/)`
    // inside a `*`) stays `Internal` (rejected) — the recognizer is the single gate, never
    // a position heuristic that could false-accept.
...
pub enum LoweringRoute {
    ...
    /// the transition — a correct fallback, never a mis-lowering. (`lark.REGEXP` is *not*
    /// here: its internal `(?!\/)` is now recognized by
    /// [`super::lower::recognize_regexp_idiom`] and lowered, so it routes to [`Self::Lowered`].)
    DeclinedToFancy {
