...
impl GrammarCompiler {
    ...
    fn resolve_import(&mut self, spec: ImportSpec) -> Result<(), GrammarError> {
        ...
        if is_library {
            if let Some(src) = bundled_grammar_source(&module_path[0]) {
                ...
                return self.import_from_source(src, None, &module_path, &names_to_import, &dotted);
            }
        }
    ...
    fn resolve_file_import(
        ...
        self.import_from_source(&text, sub_base, module_path, names_to_import, &dotted)
    }

    /// Parse a grammar `text` (from a file or a bundled library) and copy the
    /// requested terminals/rules — and, for a rule, its dependency closure — into
    /// this grammar. `sub_base` is the base directory the imported grammar's *own*
    /// relative imports resolve against (`None` for an embedded library, which can
    /// only re-import other libraries, never files).
...
/// same source-parse + closure-copy path as a sibling-file import. `python` and
/// `lark` carry a few documented lookaround-free adaptations (the `regex` crate has
/// no lookahead/lookbehind) — see the header of each bundled file. Pinned by
/// `tests/test_stdlib.rs`.
